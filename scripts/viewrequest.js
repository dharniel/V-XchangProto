...
//Gets details of request from db
function getRequestDetails() {
    return new Promise ((resolve) => {
        $.get('http://'+window.location.host+'/viewrequestpostdeets', async (data) => {
            fetchStatus(data)
            document.title = data.postCourseCode + " : " + data.postCourseTitle
            resolve(await renderRequest(data))
        })
    })
}

//Fetches comments for a specific request post
function getCommentDetails() {
    $.get('http://'+window.location.host+'/viewbookrequestcomments', (data) => {
...
//Renders the details of the request in view
function renderRequest(post){
    var poster;
    $.get('http://'+window.location.host+'/getUser', (data) => {
        poster = data;
    })
    $("#allcontainer").append(
    ...
    <div class="card-body" id="request">
        <div id="requestBody" class="card book-pill">
            <div class="card-header d-flex bg-dark">
                ...
                <span id="${post.postId}" class="text-light pt-0 ml-auto btn material-icons" onclick="processPostFollow(${post.postId})"></span>
            </div>
            <div class="card-body">
...
//Adds comments for post to view
function renderComments(post){
    ...
    if (parseTime.getHours() >= 12) {
        merdian = "PM"
    } else {
        meridian = "AM"
    }
    if (parseTime.getHours() < 10) {
        append = "0"
    }
    if (parseTime.getMinutes() < 10) {
        appendTime = "0"
    }
    var time = append + parseTime.getHours() + ":" + appendTime + parseTime.getMinutes();
    if (parseTime.getHours >= 12) {
        merdian = "PM"
    } else {
        meridian = "AM"
