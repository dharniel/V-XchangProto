...
function getPosts() {
    $.get('http://'+window.location.host+'/bookpostedit', (data) => {
        if (data.length !== 0) {
...
function addPosts(post){
    $("#allmybooks").append(
    `<div class="card book-pill mb-4" id="${post.postId}">
        ...
        <div class="card-body">
            ...
            <a href="/viewbookpostedit/${post.postId}" class="btn btn-outline-success">Edit Post</a>
            <a href="#" class="btn btn-outline-danger" data-toggle="modal" data-target="#confirm-delete" onClick="addLink(${post.postId})">Delete Post</a>
        </div>
    </div>`
    )
}
