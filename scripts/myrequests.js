...
//Get all request that a user has made
function getRequests() {
    $.get('http://'+window.location.host+'/requestpostedit', (data) => {
        if (data.length !== 0) {
            data.forEach(addRequests);
        } else {
            $("#allmyrequests").append(
...
//Add these requests to the html view. 
function addRequests(request){
    $("#allmyrequests").append(
        `<div class="card book-pill mb-4" id="${request.postId}">
            <div class="card-header d-flex">
                <span class="font-weight-bold mr-auto">${request.postCourseCode} : ${request.postCourseTitle}</span>
            </div>
            <div class="card-body">
                <h5 class="card-text font-weight-normal">${request.postDescription}</h5> 
                <a href="/viewrequestpostedit/${request.postId}" class="btn btn-outline-success">View Request</a>
                <a href="#" class="btn btn-outline-danger" data-toggle="modal" data-target="#confirm-delete" onClick="addLink(${request.postId})">Delete Request</a>
            </div>
        </div>`
    )
}
...
function addLink(postId) {
    document.getElementById("confirmDelete").setAttribute("onclick", `postDelete(${postId})`);
}
...
function postDelete(id) {
    ...
    setTimeout(() =>  {
        $("#body").append(
            `<div class="alert alert-danger fade show" style="position: fixed;top: 95%;left: 50%; z-index: 2; transform: translate(-50%, -50%);"role="alert">
                Request deleted.
            </div>`
        )
        setTimeout(() => {
    ...
    $.post('/deletepostrequest', {"postId" : id})
}
