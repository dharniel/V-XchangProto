...
function getFields() {
    $.get('http://'+window.location.host+'/fetchprofile2', (data) => {
        addToFields(data)
    })
}

//Populate fields in profile form
function addToFields(post){
    $('#firstName').val(post.first_name)
    $('#lastName').val(post.last_name)
    $('#middleInitial').val(post.middle_name)
    $('#email').val(post.email)
    $('#classyear').val(post.class_year)
}
